...
async def download_file(params: DownloadFileInput) -> str:
    ...
    Returns:
        Path to downloaded file and success message
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as http_client:
            ...
            try:
                ...
                try:
                    error_data = e.response.json()
                    return f"Error downloading file: HTTP {e.response.status_code} - {error_data.get('errorMessage', 'Unknown error')}"
                except:
                    return f"Error downloading file: HTTP {e.response.status_code} - {e.response.text[:500]}"
            ...
            work_path = f"/home/claude/{filename}"
            ...
            result_lines = [
                ...
                f"**Download Type**: {params.download_type.value}",
                f"**Saved as**: {filename}",
                f"**Size**: {_format_filesize(len(response.content))}",
            ]
